describe('Spellbook.filter', function() {
  beforeEach(function() {
    ...
    this.activeLinkClass = 'is-active';
    this.hiddenItemClass = 'is-hidden';
  ...
  it('should add an active class to the clicked link', function() {
    ...
    return expect(this.link.first()).toHaveClass(this.activeLinkClass);
  });
  it('should remove the active class from all other links', function() {
    this.link.first().click();
    return expect(this.link.not(this.link.first())).not.toHaveClass(this.activeLinkClass);
  });
  it('should show the first set when the first set link is clicked', function() {
    var firstSet;
    firstSet = $('.set-01');
    this.link.click();
    return expect(firstSet).not.toHaveClass(this.hiddenItemClass);
  });
  it('should show the second set when the second set link is clicked', function() {
    var secondSet;
    secondSet = $('.set-02');
    this.link.click();
    return expect(secondSet).not.toHaveClass(this.hiddenItemClass);
  });
  return it('should show all elements when the all link is clicked', function() {
    this.link.click();
    return expect(this.item).not.toHaveClass(this.hiddenItemClass);
  });
});
